...
inline int getFirstInitialMother(const Pythia8::Event& event, int index){
  while (event[index].mother1() > 2)
	index = event[index].mother1();
  return index;
}


void getAllInitialMothers(const Pythia8::Event& event, int index, std::set<int>& mothers){
  // trace back as far as is unambiguous
  while (event[index].mother1() > 2 && (event[index].mother2() == 0 || event[index].mother2() == event[index].mother1()))
	index = event[index].mother1();
	// reached the beams
  if (event[index].mother1() <= 2) {
	mothers.insert(index);
	return;
  }
	// reached a point with two mothers, keep tracing both
  getAllInitialMothers(event, event[index].mother1(), mothers);
  getAllInitialMothers(event, event[index].mother2(), mothers);
...
ParticleVector _convert_pythia_event(Pythia8::Pythia* pythia,
  ...
  for (int i = 0; i < event.size(); ++i) {
	...
	////	std::cout << pythia.event.motherList(i) << std::endl;
	  // if (event[i].mother1() <= 2) {
	  // 	mothers.insert(i);
	  // 	return;
	  // }

	const auto next = _convert_particle(event[i]);
	if (predicate(next)) {
		//out.push_back(next);
		...
		if(event[i].id()==25) { // it's an h0
			...
			if(did>=0 && event[did].idAbs()==13) { // and it's daughter is a muon
				...
				double ctau2=event[i].tau();
				std::cout << ctau2 << ", " << std::endl;
			}
		}

...
int main(int argc, char* argv[]) {
  ...
  option debug_opt   ('D', "debug",    "Debugging Mode",            option::no_arguments);
  ...
  const auto script_argc = -1 + util::cli::parse(argv,
    {&help_opt, &gen_opt, &det_opt, &shift_opt, &data_opt, &export_opt, &script_opt, &events_opt,
     &save_all_opt, &vis_opt, &quiet_opt, &thread_opt, &debug_opt});
  ...
  ActionInitialization::Debug = debug_opt.count ? true : false;
